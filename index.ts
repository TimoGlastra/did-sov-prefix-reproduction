...
const useLegacyDidSovPrefix = process.env.LEGACY_DID_SOV_PREFIX === "true";
...
const run = async () => {
  ...
  const schema = await agent.ledger.registerSchema({
    attributes: ["name", "age"],
    name: "hl-gbs-demo" + utils.uuid(),
    version: "10.0",
  });

  const credentiaDefinition = await agent.ledger.registerCredentialDefinition({
    schema,
  ...
  await agent.credentials.offerCredential({
    ...
    credentialFormats: {
      indy: {
        credentialDefinitionId: credentiaDefinition.id,
        attributes: [
          {
            name: "name",
            value: "Timo",
